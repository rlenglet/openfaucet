...
class MidokuraVendorHandler(object):
  ...
  def deserialize_vendor_action(self, length, buffer):
    ...
    # TODO(romain): Handle error reporting in an excetpion handler, then
    # make this into a @staticmethod which raises an exception.
    self.protocol().logger.error('unknown Midokura extension action subtype',
                                 subtype)
    self.protocol().raise_error_with_request(oferror.OFPET_BAD_ACTION,
                                             oferror.OFPBAC_BAD_VENDOR_TYPE)

  # protocol() method injected into instances by OpenflowProtocolFactory.
  # def protocol(self):
  #   """Returns the protocol object passed to the OpenflowProtocolFactory
  #      which was passed this VendorHandler."""

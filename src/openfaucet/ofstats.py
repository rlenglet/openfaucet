...
#
# Vendor stats are handled specially by the OpenflowProtocol implementation,
...
class DescriptionStats(collections.namedtuple('DescriptionStats', (
  ...
  The attributes of the description are:
    ...
    dp_desc: Human readable description of the datapath, for debugging
        purposes, e.g. 'switch3 in room 3120', as a string up to 256
        characters.
  """
  ...
  def serialize(self):
    ...
    Returns:
      ...
      OpenFlow ofp_desc_stats. The ofp_stats_reply structure is not
      included in the generated string.
    """
    return struct.pack(self.FORMAT, self.mfr_desc, self.hw_desc, self.sw_desc,
                       self.serial_num, self.dp_desc)

  @classmethod
  def deserialize(cls, buf):
    """Returns a DescriptionStats object deserialized from a sequence of bytes.

    Args:
      buf: A ReceiveBuffer object that contains the bytes that are the
          serialized form of the DescriptionStats object.
    ...
    Raises:
      ...
      bytes.
    """
    return DescriptionStats(*[s.rstrip('\x00') for s in buf.unpack(cls.FORMAT)])
...
class FlowStats(collections.namedtuple('FlowStats', (
  ...
  The attributes of the flow statistics are:
    ...
    duration_sec: Time the flow has been alive in seconds, as a 32-bit
        unsigned integer.
    ...
    priority: The priority level of the flow entry, as a 16-bit
        unsigned integer.
    idle_timeout: Number of seconds idle before the flow expires, as a
    ...
    actions: The sequence of Action* objects specifying the actions to
        perform on the flow's packets.
  """
  ...
  def serialize(self, serialize_action):
    ...
    Args:
      serialize_action: A callable with signature
          serialize_action(action) that takes an Action* object and
          returns the serialized form of the action as a sequence of
          binary strings.
  ...
  def deserialize(cls, buf, deserialize_action):
    ...
    Args:
      buf: A ReceiveBuffer object that contains the bytes that are the
          serialized form of the FlowStats object.
      deserialize_action: A callable with signature
          deserialize_action(buffer) that takes a ReceiveBuffer object and
          returns the Action* object deserialized from the buffer.
    ...
    (duration_sec, duration_nsec, priority, idle_timeout, hard_timeout, cookie,
     packet_count, byte_count) = buf.unpack(cls.FORMAT2)
    actions = []
    ...
    return FlowStats(table_id, match, duration_sec, duration_nsec, priority,
                     idle_timeout, hard_timeout, cookie, packet_count, byte_count,
                     actions)
...
class TableStats(collections.namedtuple('TableStats', (
  ...
  The attributes of the table statistics are:
    table_id: Identifier of the table, as a 8-bit unsigned integer.
    name: Name of the table, as a string up to 32 characters.
    wildcards: A Wildcards object that indicates which field wildcards
        are supported by the table: if an attribute is True, that
        attribute can be wildcarded in that table's flows entries.
    max_entries: The maximum number of entries supported in the table,
        as a 32-bit unsigned integer.
    active_count: The number of active entries in the table, as a
        32-bit unsigned integer.
    lookup_count: The number of packets looked up in the table, as a
        64-bit unsigned integer.
    matched_count: The number of packets hit the table, as a 64-bit
        unsigned integer.
  """
  ...
  def serialize(self):
    ...
    Returns:
      A binary string that is a serialized form of this object into an
      OpenFlow ofp_table_stats. The ofp_stats_reply structure is not
      included in the generated string.
    """
    return struct.pack(
  ...
  def deserialize(cls, buf):
    ...
    Args:
      buf: A ReceiveBuffer object that contains the bytes that are the
          serialized form of the TableStats object.
    ...
    Raises:
      ...
      bytes.
    """
    (table_id, name, wildcards_ser, max_entries, active_count, lookup_count,
     matched_count) = buf.unpack(cls.FORMAT)
    return TableStats(table_id, name.rstrip('\x00'),
...
class PortStats(collections.namedtuple('PortStats', (
  ...
  The attributes of the port statistics are:
    ...
    rx_packets: The number of received packets, as a 64-bit unsigned integer.
    tx_packets: The number of transmitted packets, as a 64-bit unsigned integer.
    rx_bytes: The number of received bytes, as a 64-bit unsigned integer.
    tx_bytes: The number of transmitted bytes, as a 64-bit unsigned integer.
    rx_dropped: The number of packets dropped by the receiver, as a
        64-bit unsigned integer.
    tx_dropped: The number of packets dropped by the transmitter, as a
        64-bit unsigned integer.
    rx_errors: The number of receive errors, as a 64-bit unsigned
        integer. This is a super-set of more specific receive errors
        and should be greater than or equal to the sum of all rx_*_err
        values.
    tx_errors: The number of transmit errors, as a 64-bit unsigned
        integer.
    rx_frame_err: The number of frame alignment errors, as a 64-bit
        unsigned integer.
  ...
  def serialize(self):
    ...
    Returns:
      A binary string that is a serialized form of this object into an
      OpenFlow ofp_port_stats. The ofp_stats_reply structure is not
      included in the generated string.
    """
    return struct.pack(
        self.FORMAT, self.port_no,
        self._UNAVAILABLE if self.rx_packets is None else self.rx_packets,
        self._UNAVAILABLE if self.tx_packets is None else self.tx_packets,
        self._UNAVAILABLE if self.rx_bytes is None else self.rx_bytes,
        self._UNAVAILABLE if self.tx_bytes is None else self.tx_bytes,
        self._UNAVAILABLE if self.rx_dropped is None else self.rx_dropped,
        self._UNAVAILABLE if self.tx_dropped is None else self.tx_dropped,
        self._UNAVAILABLE if self.rx_errors is None else self.rx_errors,
        self._UNAVAILABLE if self.tx_errors is None else self.tx_errors,
        self._UNAVAILABLE if self.rx_frame_err is None else self.rx_frame_err,
        self._UNAVAILABLE if self.rx_over_err is None else self.rx_over_err,
        self._UNAVAILABLE if self.rx_crc_err is None else self.rx_crc_err,
        self._UNAVAILABLE if self.collisions is None else self.collisions)
  ...
  def deserialize(cls, buf):
    ...
    Args:
      buf: A ReceiveBuffer object that contains the bytes that are the
          serialized form of the PortStats object.
    ...
    """
    (port_no, rx_packets, tx_packets, rx_bytes, tx_bytes, rx_dropped,
     tx_dropped, rx_errors, tx_errors, rx_frame_err, rx_over_err, rx_crc_err,
     collisions) = buf.unpack(cls.FORMAT)
    return PortStats(
        port_no,
...
class QueueStats(collections.namedtuple('QueueStats', (
  ...
  def serialize(self):
    ...
    Returns:
      A binary string that is a serialized form of this object into an
      OpenFlow ofp_queue_stats. The ofp_stats_reply structure is not
      included in the generated string.
    """
    return struct.pack(self.FORMAT, self.port_no, self.queue_id, self.tx_bytes,
  ...
  def deserialize(cls, buf):
    ...
    Args:
      buf: A ReceiveBuffer object that contains the bytes that are the
          serialized form of the QueueStats object.

...
class ReceiveBuffer(object):
  """A byte buffer that supports efficient struct unpacking and bytes skipping.
  """

  # TODO(romain): Internally store the bytes as a sequence of arrays, for
  # efficiency.
  ...
  def set_message_boundaries(self, length):
    ...
    Set the current offset as the start of the next message.  Set the
    message length to the given length.  The length is decremented in
    every read done by calling unpack() or read_bytes().

    Args:
      length: The maximum number of bytes that can be read for the
          next message. Must be lower or equal to the length of this
          buffer, i.e. the number of unread bytes.
  ...
  def get_first_message_bytes(self, num):
    ...
    Returns:
      An array of up to num bytes. The actual number of bytes may be
  ...
  def compact(self):
    ...
    After calling this method, set_message_boundaries() must be called
    to correctly reset the message boundaries before reading any bytes
    from this buffer.
    """
    self._msg_start -= self._offset  # TODO(romain): May be <0 which is wrong.
  ...
  def unpack(self, format):
    ...
    Args:
      format: The struct format string to use to unpack the bytes.
  ...
  def read_bytes(self, num):
    ...
    Args:
      num: The number of bytes to get. Must be lower or equal than the
          limit and the number of unread bytes.

...
class MockDelayedCall(object):
    ...
    def cancel(self):
        ...
        Raises:
            twisted.internet.error.AlreadyCalled: The call has already
                happened.
            twisted.internet.error.AlreadyCancelled: The call has
    ...
    def delay(self, secondsLater):
        ...
        Args:
            secondsLater: How many seconds from its current firing
                time to delay.
    ...
    def active(self):
        ...
        Returns:
            True if this call is still active, False if it has been
            called or cancelled.
        """
...
class MockReactorTime(object):
    ...
    January 1, 1970, that is initially set to zero.  This clock must
    be explicitly incremented by clients, by calling increment_time(),
    to simulate the passing of time in the Memcached server.  Calling
    ...
    def callLater(self, delay, callable, *args, **kw):
        ...
        Returns:
            An object which provides IDelayedCall and can be used to
            cancel the scheduled call, by calling its cancel()
            method. It also may be rescheduled by calling its delay()
            or reset() methods.
        """
    ...
    def getDelayedCalls(self):
        ...
        Returns:
            A tuple of all IDelayedCall providers representing all
            currently scheduled calls. This is everything that has
            been returned by callLater but not yet called or canceled.
        """
        ...
        for delayed_call in self.delayed_calls:
            if delayed_call.active():
                if delayed_call.getTime() <= self.time:
                    ...
                    delayed_call.callable(*delayed_call.args,
                                          **delayed_call.kw)
                else:
